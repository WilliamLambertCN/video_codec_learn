...
with open(f"{args.input_file}_yuvinfo_{args.task}.txt", 'w') as f_yuvinfo:
    for idx in range(1,total_frames+1):
        ...
        #         p_layers["p5"].squeeze()]
        # # feat = [p_layers["p5"].squeeze()] #p5 shape() CWH
        # width_list = [16, 32, 64, 128]
        # height_list = [16, 8, 4, 2]
        
        feat = [p_layers["p5"].squeeze()]
        # feat = [p_layers["p5"].squeeze()] #p5 shape() CWH
        width_list = [16,]
        height_list = [16,]
        ...
        plane = torch.empty((0, feat[0].shape[2] * width_list[0])).cuda()
        for blk, width, height in zip(feat, width_list, height_list):
            big_blk = torch.empty((0, blk.shape[2] * width)).cuda()
            for row in range(height):
                big_blk_col = torch.empty((blk.shape[1], 0)).cuda()
                for col in range(width):
